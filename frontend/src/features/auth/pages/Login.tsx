...
export default function Login() {
  ...
  const onSubmit = async (d: Form) => {
    try { await login(d.email, d.password); toast.success("Bem-vindo!"); nav(redirect); }
    catch (e: any) { toast.error(e.response?.data?.error?.message ?? "Falha no login"); }
  };
    ...
    <div className="max-w-md mx-auto px-4 py-12">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl shadow-sm p-6 space-y-4">
        ...
        <p className="text-sm text-center text-slate-600">
          Não tem conta? <Link to="/cadastro" className="text-primary-600 hover:underline">Cadastre-se</Link>
        </p>
      </form>
    </div>
  );
